...
7.函数扩展
    参数默认值：
        es6之前无法直接在函数传入参数时指定默认值，es6可以，直接写在参数定义的后面
        ...
        参数默认已经声明，不可以再次在函数内部用let或者const声明
        
        参数默认值与解构赋值结合（传参是一个对象）：
            function foo({x, y = 5} = {}) {
                console.log(x, y);
            }
        参数默认值的位置：一般放在参数末尾
        ...
        参数作用域
            定义默认值的参数，在初始化的时候会形成一个独立作用域，初始化完毕作用域消失
        应用：
            利用参数默认值规定某个参数必须传入，否则报错
    ...
    rest参数（形式为...变量名）用于获取函数的多余参数，这样就不需要使用arguments对象了。
        ...
        rest 参数之后不能再有其他参数，即它前面可以有参数，后面不可以
    ...
    尾调用优化：
        尾调用：指某个函数的最后一步是调用另一个函数，函数式编程重要概念
        尾调用优化：只保留内层函数的调用帧：如果函数g不是尾调用，函数f就需要保存内部变量m和n的值、g的调用位置等信息。但由于调用g之后，函数f就结束了，所以执行到最后一步，完全可以删除f(x)的调用帧，只保留g(3)的调用帧
